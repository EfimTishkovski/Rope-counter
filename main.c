...
const float k = 0.2198;        // Коэфициент для пересчёта длины нужно 0.205 по результатам испытаний
...
void timer_ini(void)
{
	...
	OCR1AL = 0b01000010;   // корректировка конечное число 3906 (8 000 000 / 256 / 8) повышаем частоту в 8 раз
...
int main(void)
{
	...
	timer_ini();
	DDRD = 0xFF;               // Порт B весь на выход  0b A B C D E F G DP
	...
	show_number(888);  // Проверка индикатора
	_delay_ms(500);   
	show_number(451);
	_delay_ms(500);
	main_counter = 0;
    ...
    while (1) 
    {
		if (main_counter > 999)
		{
			main_counter = 0;       // Обнуление при переполнении
		}
		...
		// Сброс
		if (!(PINB&0b0000001)) //(работает по инверсии подключен к минусу)
		{
			if (butstate_reset == 0)
			{
				main_counter = 0;
